...
// ========================================
// For Firebase JS SK v7.20.0 and later, measurementId is optional
...
// ========================================
// ✅ FETCH USER COMPLETED SONGS (ONE DOCUMENT!)
// ========================================
async function fetchUserCompletedSongs() {
    if (!currentUser) return {};

    try {
        // ✅ ONE document per user - musicCompletions/{userId}
        const completionDoc = await db.collection('musicCompletions')
            .doc(currentUser.uid)
            .get();
        
        if (completionDoc.exists) {
            return completionDoc.data().songs || {};
        }
        return {};
    } catch (error) {
...
// ========================================
// ✅ MARK SONG AS COMPLETED (ONE DOCUMENT!)
// ========================================
async function markSongAsCompleted(songId, earnings, listeningTime) {
    if (!currentUser) return;

    try {
        // ✅ Update ONE document with all songs
        const completionRef = db.collection('musicCompletions').doc(currentUser.uid);
        ...
        console.log("✅ Song marked as completed:", songId);
    } catch (error) {
        console.error("❌ Error marking song as completed:", error);
    }
}
...
function extractThumbnailFromUrl(url) {
    ...
    return 'avatar.png';//'https://via.placeholder.com/800x300/10B981/FFFFFF?text=Music';
}
...
async function fetchSongs() {
    try {
        ...
        const completedSongs = await fetchUserCompletedSongs();
        songs = [];
...
async function claimEarnings() {
    ...
    try {
        ...
        // ✅ Mark as completed (ONE document)
        await markSongAsCompleted(playerState.currentSong.id, earningsToAdd, playerState.listeningDuration);
...
async function initMusicPage() {
    try {
        ...
        console.log("✅ Music & Earn Page initialized successfully");
